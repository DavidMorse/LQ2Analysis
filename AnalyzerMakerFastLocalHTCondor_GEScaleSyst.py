...
# Create output directory for nominal trees (icopy = 50) and one for each GE copy (icopy = 0 to 49)
thiseos = []
for icopy in range(51):
	thiseos.append(thisdir+'/'+c2file.replace('.py','')+'_'+tagname+'_GEcopy'+str(icopy)+'_'+now)
...
if override_dir != '':
	...
	nowSplit = (override_dir.replace('\n','').split('_GEcopy')[-1]).split('_')[1:]
	now = '_'.join(nowSplit)
	thiseos = []
	for icopy in range(51):
		thiseos.append(override_dir.split('_GEcopy')[0]+'_GEcopy'+str(icopy)+'_'+now)
	#print now
	#exit()

for icopy in range(51):
	os.system(' mkdir '+ thiseos[icopy])
print ' \n-------------------------------------------------------------------'
bjobs = []
...
for icopy in range(51):
	jobs[str(icopy)] = []
# Create subfolders for all 51 directories (nominal + 50 copies)
# output_subfolders is a dictionary with key = main directory (element of thiseos) and value = list of subdirectories
output_subfolders = {}
for icopy in range(51):
	output_subfolders[thiseos[icopy]] = [thiseos[icopy]+'/outputdir'+str(random.randint(1,999999999)) for nnn in range(20)]
	for f in output_subfolders[thiseos[icopy]]:
		os.system('mkdir '+f)
...
for x in range(len(SignalType)):
	...
	for icopy in range(51):
		for f in fcont:
			jobs[str(icopy)].append('python '+pyfile.replace('\n','')+' -f '+f.replace('\n','').replace('//','/')+' -s '+sigma+' -n '+Norig+' -j '+thisjson + ' -l 1.0 -p '+dopdf+' -y '+year+' -c '+str(icopy)+' -d '+random.choice(output_subfolders[thiseos[icopy]]).replace('\n',''))

# Some NTuple sets have larger files then others. Avoid grouping of large files by shuffling. 
for icopy in range(51):
	random.shuffle(jobs[str(icopy)])

print 'Total file count:',len(jobs['0'])
...
def MakeJobs(njobs,GEcopy):
	Nj = 0

	jlist = []
	jstr = ''
	jstr = str(os.popen('find '+thiseos[GEcopy]).readlines())
	for j in jobs[str(GEcopy)]:
		#copy = int((j.split(' -c ')[-1]).split(' ')[0])
		...
		filesig = filesig+'_' #+'GEcopy'+str(GEcopy)+'_'
                #print filesig
		#print jstr
	...
	findircont = str(os.popen('ls '+thiseos[GEcopy]).readlines())
	# if '.txt' in findircont or '.root' in findircont:
	# 	FolderizeOutput(thiseos)

	# FolderizeOutput(thiseos)
	
	os.system('rm '+thiseos[GEcopy]+'/subber_*.tcsh')
	os.system('rm '+thiseos[GEcopy]+'/subber_*.sub')
	for j in jobgroups:
		Nj += 1
		subber = open(thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.tcsh','w')
		...
		subber = open(thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.sub','w')
		subber.write('executable            = '+thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.tcsh')
		...
		#testmatch    = 3 days
		#nextweek     = 1 week
		subber.write('\noutput                 = '+thiseos[GEcopy]+'_'+str(Nj))
		subber.write('\nerror                  = '+thiseos[GEcopy]+'_'+str(Nj))
		subber.write('\nlog                    = '+thiseos[GEcopy]+'_'+str(Nj)+'.log')
		...
		os.system('chmod 777 '+thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.tcsh')
		os.system('chmod 777 '+thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.sub')
		...
		#Switch to HTCondor
		print 'condor_submit '+thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.sub'
		os.system( 'condor_submit '+thiseos[GEcopy]+'/subber_'+now+'_'+str(Nj)+'.sub')
		os.system('sleep 0.4')
		# sys.exit()
...
while keep_going != 0:
	...
	keep_going = 0
	for icopy in range(51):
		keep_going += MakeJobs(nsplit,icopy)
	if keep_going == 0:
		break
	...
	while done!=1:
		...
		#	 break
for icopy in range(51):
	os.system('rm '+thiseos[icopy]+'/subber_*tcsh')
	os.system('mkdir '+thiseos[icopy]+'/logs')
	os.system('mv '+thiseos[icopy]+'_* '+thiseos[icopy]+'/logs/')

#sys.exit()
findircont =[]
for icopy in range(51):
	findircont.append(str(os.popen('ls '+thiseos[icopy]).readlines()))
	if '.txt' in findircont[icopy] or '.root' in findircont[icopy]:
		FolderizeOutput(thiseos[icopy])
...
def splithadd(hstring,icopydir):
	...
	for hh in haddouts:
		finalhadd += ' '+hh
	os.system( finalhadd )
	os.system('rm '+icopydir+'/SummaryFiles/*part*root')

# sys.exit()
eosrootpath = '/eos/user/g/gmadigan/MES_Syst_Study'
if 'Analyz' in pyfile:
	for icopy in range(51):
		localicopydir = (thiseos[icopy].rstrip('/')).split('/')[-1]
		eosicopydir = eosrootpath+'/'+localicopydir
		print 'Copying ',thiseos[icopy],'\ninto ',eosrootpath,'...'
		os.system('cp -r '+thiseos[icopy]+' '+eosrootpath)
		print 'Done copying\n'
		print 'Merging files in new eos directory: ',eosicopydir
		os.system('mkdir '+eosicopydir+'/SummaryFiles')	
		groups = []
		for x in range(len(SignalType)):
		...
		for g in groups:	
			...
			allfiles = []
			haddstring = 'hadd '+eosicopydir+'/SummaryFiles/'+g+'.root'
			...
			for x in range(len(SignalType)):
				...
				signifier = eosicopydir+'/output*/*'+SignalType[x]+'*.root'
			...
			splithadd(haddstring,eosicopydir)
			os.system('rm '+eosicopydir+'/SummaryFiles/*part*root')

			print ('\n\n'+140*'*'+ '\n\n      Analysis Complete. A full set of output files can be found in  \n\n       '+eosicopydir+'/SummaryFiles\n')
			os.system('ls '+eosicopydir+'/SummaryFiles')
			print ('\n\n'+140*'*'+ '\n\n')